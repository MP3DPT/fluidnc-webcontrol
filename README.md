...
![Demo: tour the sidebar, generate a toolpath, load a job, run it, and park — all on real FluidNC hardware](docs/screenshots/demo.gif)

A tour of the sidebar (Tools, Plugins, Settings, Logs, About), generating a facing toolpath with the Surfacing/Facing tool, then loading a G-code file from the File Manager and streaming it start to finish on a real PiBot V4.96 PRO — live toolpath, live coordinates, live machine state throughout — and finishing with a one-click Park move. Recorded as a bare-machine test run (no endmill installed, Smart Plug Control disabled so the spindle never powers on) rather than an actual cut.
