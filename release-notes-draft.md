...
- v0.4.1 and this v0.4.2 are small follow-ups to v0.4.0 specifically to validate that update mechanism end-to-end on real hardware. v0.4.1's own update button didn't work - the app's release zip was being fetched from the browser, and GitHub's archive-download endpoint doesn't allow that (no CORS headers); v0.4.2 moves that download server-side, where it isn't subject to the browser's CORS rules at all. A v0.4.1 update attempt fails safely before touching anything (the request never leaves the browser), so there's nothing to clean up if you hit it - just try again once you're on a build with this fix.
...
- The in-app updater's first real-hardware attempt (v0.4.0 → v0.4.1) surfaced the CORS bug described above - real testing doing exactly what it's for. **Still not yet confirmed working end-to-end** - this v0.4.2 release exists to retry that same real update, through the button, with the fix in place, before it's something anyone should rely on for a production update.
